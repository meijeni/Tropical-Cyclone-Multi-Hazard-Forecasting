...
plots_folder = 'validation_plots_se_stormsurges'
...
def objective(trial):
    ...
    model = SDS_SENet(img_channel=X_train.shape[1], dropout_rate=dropout_rate, activation=activation).to(device)
...
class ExtendedTrainer(Trainer):
    def __init__(self, model, optimizer, criterion, train_loader, val_loader):
        ...
        self.best_model_path = 'best_model_stormsurges_se.pth'
...
def test_storm_surge_model(model, test_loader, criterion, dataset_name="test"):
    ...
    best_model_path = f'best_model_stormsurges_se.pth'
    ...
    plt.title(f'Predicted vs Actual Storm Surge Values by {dataset_name.capitalize()} Index')
    plt.xlabel(f'{dataset_name.capitalize()} Sample Index')
    plt.ylabel('Storm Surge (m)')
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(plots_folder, f'predicted_vs_actual_timeseries_{dataset_name}.png'), dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.title(f'Predictions vs Actuals (Best Storm Surge Model) - {dataset_name.capitalize()}')
    plt.xlabel('Actual Storm Surge Values')
    plt.ylabel('Predicted Storm Surge Values')
    plt.grid(True)
    plt.savefig(os.path.join(plots_folder, f'predictions_vs_actuals_scatter_{dataset_name}.png'), dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.title(f'Actual vs Predicted Storm Surge - {dataset_name.capitalize()}')
    plt.xlabel('Sample Index')
    plt.ylabel('Storm Surge (m)')
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(plots_folder, f'actual_vs_predicted_storm_surge_{dataset_name}.png'), dpi=300, bbox_inches='tight')
    plt.show()
