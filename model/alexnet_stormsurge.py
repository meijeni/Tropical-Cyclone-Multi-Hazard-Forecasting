...
plots_folder = 'validation_plots_alex_stormsurge_pacific'
...
class AlexNetStorm(nn.Module):
    def __init__(self, num_features, dropout_rate=0.5, activation='relu'):
        ...
        self.classifier = nn.Sequential(
            ...
            nn.Linear(4096, 1024),
            self.activation,
            nn.Dropout(dropout_rate * 0.6),  # Slightly less dropout for the final layer
            nn.Linear(1024, 1),
        )
...
def objective(trial):
    ...
    model = AlexNetStorm(num_features=X_train.shape[1], 
...
class ExtendedTrainer(Trainer):
    def __init__(self, model, optimizer, criterion, train_loader, val_loader):
        ...
        self.best_model_path = 'best_model_stormsurges_alexnet.pth'
...
def evaluate_model(num_epochs, train_losses, val_losses):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(plots_folder, 'training_validation_loss_alexnet.png'), dpi=300, bbox_inches='tight')
    plt.show()
...
def test_storm_surge_model(model, test_loader, criterion, dataset_name="test"):
    ...
    best_model_path = f'best_model_stormsurges_alexnet.pth'
    ...
    plt.title(f'Predicted vs Actual Storm Surge Values by {dataset_name.capitalize()} Index (AlexNet)')
    plt.xlabel(f'{dataset_name.capitalize()} Sample Index')
    plt.ylabel('Storm Surge (m)')
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(plots_folder, f'predicted_vs_actual_timeseries_{dataset_name}_alexnet.png'), dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.title(f'Predictions vs Actuals (Best Storm Surge Model - AlexNet) - {dataset_name.capitalize()}')
    plt.xlabel('Actual Storm Surge Values')
    plt.ylabel('Predicted Storm Surge Values')
    plt.grid(True)
    plt.savefig(os.path.join(plots_folder, f'predictions_vs_actuals_scatter_{dataset_name}_alexnet.png'), dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.title(f'Actual vs Predicted Storm Surge (AlexNet) - {dataset_name.capitalize()}')
    plt.xlabel('Sample Index')
    plt.ylabel('Storm Surge (m)')
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(plots_folder, f'actual_vs_predicted_storm_surge_{dataset_name}_alexnet.png'), dpi=300, bbox_inches='tight')
    plt.show()
