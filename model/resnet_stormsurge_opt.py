...
plots_folder = 'validation_plots_resnet'
...
class ExtendedTrainer(Trainer):
    def __init__(self, model, optimizer, criterion, train_loader, val_loader):
        ...
        self.best_model_path = 'best_model_stormsurges_resnet.pth'
...
train_losses, eval_losses, (val_predictions, val_actuals) = trainer.train(num_epochs=25)
...
def evaluate_model(num_epochs, train_losses, val_losses):
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(plots_folder, 'training_validation_loss_resnet.png'), dpi=300, bbox_inches='tight')
    plt.show()
...
def test_storm_surge_model(model, test_loader, criterion, dataset_name="test"):
    ...
    best_model_path = f'best_model_stormsurges_resnet.pth'
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig(os.path.join(plots_folder, f'predicted_vs_actual_timeseries_{dataset_name}_resnet.png'), dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.grid(True)
    plt.savefig(os.path.join(plots_folder, f'predictions_vs_actuals_scatter_{dataset_name}_resnet.png'), dpi=300, bbox_inches='tight')
    plt.show()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(plots_folder, f'actual_vs_predicted_storm_surge_{dataset_name}_resnet.png'), dpi=300, bbox_inches='tight')
    plt.show()
...
zero_indices_atlantic = torch.where(atlantic_targets == 0)[0]
print(f"Found {len(zero_indices_atlantic)} samples with zero targets")
print(f"Zero indices: {zero_indices_atlantic}")

# Delete those indices from atlantic_features and atlantic_targets
atlantic_features = torch.index_select(atlantic_features, 0, torch.tensor([i for i in range(atlantic_features.shape[0]) if i not in zero_indices_atlantic]))
atlantic_targets = torch.index_select(atlantic_targets, 0, torch.tensor([i for i in range(atlantic_targets.shape[0]) if i not in zero_indices_atlantic]))
